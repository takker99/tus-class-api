...
export async function postToCLASS(
  pathname: string,
  formData: { key: string; value: string }[] | Record<string, string>,
  cookie: Cookie,
) {
...
export async function goDetailedInfoPage(
  id: number,
  { comSunFacesVIEW, jSessionId }: Auth,
) {
  return await postToCLASS("/up/faces/up/po/Poa00601A.jsp", [
    {
      key:
        `form1:Poa00201A:htmlParentTable:${id}:htmlDisplayOfAll:0:allInfoLinkCommand`,
      value: "",
    },
...
const nextButtonId =
  "form1:Poa00201A:htmlParentTable:htmlDetailTbl2:deluxe1__pagerNext";
...
export async function goSyllabusList(
  page: number,
  { comSunFacesVIEW, jSessionId }: Auth,
) {
