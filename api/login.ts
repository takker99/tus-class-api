    ...
    const announceSummary = [];
    {
        ...
        for (const index of range(announceTable.children[0].children.length)) {
            announceSummary.push({
                ...
                count: parseInt(dom.getElementById(`${baseId}:${index}:htmlDisplayOfAll:0:htmlCountCol21702`)?.textContent?.replace(/全(\d+)件/, '$1') ?? ''),
                hasHiddenAnnounces: dom.getElementById(`${baseId}:${index}:htmlDisplayOfAll:0:htmlCountCol21702`)?.nextElementSibling !== null,
                id: index,
            });
        }
    }
